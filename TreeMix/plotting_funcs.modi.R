...
plot_tree_internal = function(d, e, o = NA, cex = 1, disp = 0.005, plus = 0.005, arrow = 0.05, ybar = 0.01, scale = T, mbar = T, mse = 0.01, plotmig = T, plotnames = T, xmin = 0, lwd = 1, font = 1){
	...
	if (scale){
	print (paste("mse", mse))
        lines(c(0+0.6, mse*10+0.6), c(ybar, ybar))
	text( 0+0.6, ybar - 0.04, lab = "10 s.e.", adj = 0, cex  = 0.8)
	lines( c(0+0.6, 0+0.6), c( ybar - 0.01, ybar+0.01))
	lines( c(mse*10+0.6, mse*10+0.6), c(ybar- 0.01, ybar+ 0.01))
	}
        if (mbar){
                mcols = rev( heat.colors(150) )
                ...
                w = l/100
                xma = max(d$x/20)
                rect( rep(0, 100)+0.6, ymi+(0:99)*w, rep(xma, 100)+0.6, ymi+(1:100)*w, col = mcols, border = mcols)
                text(xma+disp+0.6, ymi, lab = "0", adj = 0, cex = 0.9)
		if ( mw >0.5){ text(xma+disp+0.6, yma, lab = "1", adj = 0, cex = 0.9)}
                else{
			text(xma+disp+0.6, yma, lab = "0.5", adj = 0, cex =0.9)
		}
		text(0.6, yma+0.06, lab = "Migration", adj = 0 , cex = 0.9)
		text(0.6, yma+0.03, lab = "weight", adj = 0 , cex = 0.9)
        }	
}
