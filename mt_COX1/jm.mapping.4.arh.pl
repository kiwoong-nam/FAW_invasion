...
my $fqD="/home/knam/work/processing_fq/laneseq_201909/processed/ARH";
opendir my $D,$fqD;
my @files=readdir($D);
close $D;

my @samples;
foreach my $file (@files) {if($file=~/ARH/) {push @samples,$file}}
...
/home/knam/save/programs/bowtie2-2.3.4.1-linux-x86_64/bowtie2 -x sfmt -1 /home/knam/work/processing_fq/laneseq_201909/processed/ARH/SAMPLE/SAMPLE.pair1.truncated.gz -2 /home/knam/work/processing_fq/laneseq_201909/processed/ARH/SAMPLE/SAMPLE.pair2.truncated.gz | /home/knam/save/programs/samtools-1.9/samtools view -F 4 -b -h -o ../bam.4/SAMPLE.raw.bam
...
foreach my $sp (@samples)
{
	...
	open my $fd,">$outputD/j.arh$n";
	print $fd $j;
	close $fd;

	print "sbatch $outputD/j.arh$n\n";
	$n++;
}
